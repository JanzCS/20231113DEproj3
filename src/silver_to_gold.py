...
from pyspark.sql.functions import monotonically_increasing_id, explode, sequence, to_date
...
county_dim = county_dim.join(education_ext, on = (county_dim.county_fips == education_ext.fips_code)).drop('fips_code')
county_dim = county_dim.join(gdp_ext, on = (county_dim.county_fips == gdp_ext.fips_code)).drop('fips_code')
county_dim = county_dim.join(pop_est_ext, on = (county_dim.county_fips == pop_est_ext.fips_code)).drop('fips_code')
county_dim = county_dim.join(pres_ext, on = 'county_fips')
county_dim = county_dim.drop('county_name', 'county', 'state')

display(education_ext)
...
combined_cd = combined_cd.join(cd_house_ext, on = (combined_cd.cd_original == cd_house_ext.congressional_district)).drop('congressional_district')
