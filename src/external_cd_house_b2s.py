...
house_df = (
    ...
    .select("year", "state_po", "district", "candidate", "candidatevotes", "party")
...
winner_df = (
    house_df.groupBy("year", "state_po", "district", "candidate", "party")
    .agg(max("candidatevotes").alias("votes"))
    .orderBy("year", "state_po", "district", desc("votes"))
    .withColumn(
        "rank",
        rank().over(
            Window.partitionBy("year", "state_po", "district").orderBy(desc("votes"))
        ),
    )
...
winner_2018_df = winner_df.filter(col("year") == 2018).withColumnRenamed(
    "party", "party_2018"
)
winner_2020_df = winner_df.filter(col("year") == 2020).withColumnRenamed(
    "party", "party_2020"
)
winner_2022_df = winner_df.filter(col("year") == 2022).withColumnRenamed(
    "party", "party_2022"
)
...
winner_joined_df = (
    winner_2018_df.join(winner_2020_df, ["state_po", "district"], how="outer")
...
winner_joined_df = winner_joined_df.select(
    concat_ws("-", "state_po", lpad(col("district"), 2, "0")).alias(
...
winner_joined_df.repartition(1).write.mode('overwrite').parquet(external_location)
