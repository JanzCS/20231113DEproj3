...
education = education.withColumnRenamed(
    "Federal Information Processing Standard (FIPS) Code", "fips_code"
).withColumn("fips_code", lpad(col("fips_code"), 5, "0"))

# Remove commas and change fields from string to int
education = (
    education.withColumn(
        "Less than a high school diploma, 2017-21",
        regexp_replace("Less than a high school diploma, 2017-21", ",", "").cast(
            IntegerType()
        ),
    )
    .withColumn(
        "High school diploma only, 2017-21",
        regexp_replace("High school diploma only, 2017-21", ",", "").cast(
            IntegerType()
        ),
    )
    .withColumn(
        "Some college or associate's degree, 2017-21",
        regexp_replace("Some college or associate's degree, 2017-21", ",", "").cast(
            IntegerType()
        ),
    )
    .withColumn(
        "Bachelor's degree or higher, 2017-21",
        regexp_replace("Bachelor's degree or higher, 2017-21", ",", "").cast(
            IntegerType()
        ),
    )
)
...
# Rename Columns
education = (
    education.withColumnRenamed(
        "Less than a high school diploma, 2017-21", "less_than_hs_17-21"
    )
    .withColumnRenamed("High school diploma only, 2017-21", "hs_only_17-21")
    .withColumnRenamed(
        "Some college or associate's degree, 2017-21", "some_college_17-21"
    )
    .withColumnRenamed("Bachelor's degree or higher, 2017-21", "bach_or_higher_17-21")
    .withColumnRenamed(
        "Percent of adults with less than a high school diploma, 2017-21",
        "perc_less_than_hs_17-21",
    )
    .withColumnRenamed(
        "Percent of adults with a high school diploma only, 2017-21",
        "perc_hs_only_17-21",
    ...
    .withColumnRenamed(
        "Percent of adults completing some college or associate's degree, 2017-21",
        "perc_some_college_17-21",
    )
    .withColumnRenamed(
        "Percent of adults with a bachelor's degree or higher, 2017-21",
        "perc_bach_or_higher_17-21",
    )
)
...
external_location = f"abfss://{silver_cont_name}@{storage_acct_name}.dfs.core.windows.net/{location_from_container}external/county/education"

# two dataframes for each file type
education.repartition(1).write.mode('overwrite').parquet(external_location)
